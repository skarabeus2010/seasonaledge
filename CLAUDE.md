...
> Version 60.1 | 2026-09-11 | **Endabnahme Flows/Optionen (Detail: [docs/OPTIONS.md](docs/OPTIONS.md#endabnahme-2026-09-11-pr-275), PR #275)** — 6 der 8 Fixes halten, **2 waren lückenhaft**. **`--vol-pctl` griff nur am CLI:** der Parser stand auf 0.5, aber **vier Funktionssignaturen** defaulteten weiter auf 0.0 (`_leg_ivs`, `_reconstruct`, `run_ticker`, `verify`) — ein Direktaufruf hätte still eine anders gefilterte Reihe erzeugt. Jetzt kommen alle aus `shared/black_scholes.VOL_PCTL`. **Fix 3 war nur im Live-Pfad angewendet:** dort prüft die *Aufrufstelle*, ob `cm_iv_atm` da ist; der Backfill setzte `cm_mode` auch ohne `iv_atm` → das Frontend rankte die Zeile und fiel auf `(call_iv−put_iv)/2` zurück, die `put_zeta = −call_zeta` erzwingt und den Quadranten auf die Antidiagonale kollabieren lässt. Solche Zeilen verlieren jetzt ihre cm-Kennzeichnung (`noatm`). **Eine Reviewer-Einstufung korrigiert:** `_leg_own` vs. `_leg_ivs` sei ein echter Rechenfehler, weil die Ausdünnung die liquiden Strikes am Spot bevorzuge — **der Mechanismus stimmt nicht**, ausgedünnt wird *gleichmäßig* über das ±30 %-Band. Gemessen (4000 Simulationen): die Volumenschwelle liegt im Backfill **~7 % (Median)** höher, 55 % der Fälle positiv — klein, aber systematisch. Unter dem unterstellten Mechanismus wären es +9624 % gewesen. Bleibt offen, jetzt mit Zahl statt Vermutung. **Lesson (zum zweiten Mal):** ein Fix an der **Aufrufstelle** schützt nur diese eine Stelle — die Regel gehört zum **Erzeuger**, sonst fehlt sie beim nächsten Aufrufer.
> Version 60.0 | 2026-09-11 | **Kern-Methodik FREIGEGEBEN + direkter Codex-Kanal (kanonisch: [docs/KERNMETHODIK.md](docs/KERNMETHODIK.md) 4d, Betrieb: [docs/REVIEW_MIT_CODEX.md](docs/REVIEW_MIT_CODEX.md), PR #274)** — **Abnahme grün:** beide Auflagen erfüllt, Freigabe-Bedingung gerechnet. Der Reviewer hatte sich EINE Zahl ausbedungen — die XETRA-2023-Jahresrendite aus **Roh**-Schlusskursen, weil sie Referenz, Constant-Fill und Jahresendgrenze gleichzeitig prüft: SAP.DE **+45,61469156 % Rohkurs vs. +45,61469156 % Pipeline, Differenz 0,0000000000 pp** (BMW.DE und SPY ebenso). Der Fall ist der kritische: **XETRA 2023 endet am 29.12. = Kalendertag 363** — die ursprüngliche Regel `< 365` hätte dieses vollständige Jahr verworfen. **Fail-open geschlossen:** JS fiel bei fehlendem `last_actual_day` auf 365 zurück und wies das Jahr als vollständig aus, während Python es verwarf → beide jetzt fail-closed; dabei fiel auf, dass `buildExtendedYearData` im Gegensatz zu `buildYearData` nicht auf 365 klammerte (Schaltjahr). **Mutationstest transaktionssicher** (Lock, Bytes-I/O, atomares Ersetzen, nachgewiesene Wiederherstellung) und von 12 auf **15 Mutationen**: alle drei vom Reviewer vorgeschlagenen wurden vorher **nicht** gefangen (`calculate_period_stats`-WERT statt Anzahl, `buildMonthlyStats`, `buildTOMHeatmap`) — plus zwei selbst gefundene. **Direkter Codex-Kanal steht** (`codex exec -s read-only`, CLI-Upgrade auf 0.154.0 nötig): kein Copy-Paste mehr. **Lessons:** **vier von fünf „KEINE FREIGABE" waren Werkzeugfehler, kein Urteil** — vor der Interpretation prüfen, ob überhaupt etwas lief; `nohup … &` schneidet den Lauf ab (Vordergrund nutzen); `py -3.14` ist für Codex unsichtbar (absoluter Pfad); der Mutationstest **schreibt** und kann unter `-s read-only` gar nicht laufen; **Beweiszahlen in voller Genauigkeit zeigen** — gerundete Eingangskurse neben präzisem Ergebnis erzeugten einen Scheinwiderspruch von 0,0026 pp.
> Version 59.0 | 2026-09-11 | **Dritte Prüfrunde: Zirkelschluss in der Jahresreferenz + Wächter beweisbar rot (kanonisch: [docs/KERNMETHODIK.md](docs/KERNMETHODIK.md) 4c, PR #273)** — 3 Korrekturen bestätigt, 4 unvollständig, 6 neue Befunde; alle berechtigt, alle behoben. **Der schwerste traf wieder den eigenen Fix:** `year_end_reference` nahm das Maximum über ALLE Jahre und **beglaubigte die Daten mit sich selbst** — nur das laufende Jahr (Tag 250) → ref=250 → galt als vollständig; alle Jahre abgeschnitten → alle vollständig; Dezember-Delisting (Tag 349) allein → vollständig. Jetzt zwei Sperren: **nur abgeschlossene Jahre stiften die Referenz**, und sie muss nach einem Jahresende aussehen — Untergrenze **gemessen**, nicht geraten (frühestes Jahresende über alle zehn Börsenkalender 1990-2030 = Tag 362 → `JAHRESENDE_UNTERGRENZE = 359`). Beim Testen fiel ein **zweiter, nicht gemeldeter** Fehler auf: bei `ref = 0` winkte `lad >= ref` weiter jedes Jahr durch. Zyklus-Signifikanz prüfte die Mindestanzahl **vor** dem Filter (3 Kandidaten → 2 Beobachtungen im Test). **Wächter:** fehlendes node liess den Lauf **grün** durchgehen (jetzt Fehlschlag, bewusst per `--ohne-node`); der ToM-Test prüfte nur `curve[2] == 5` und hätte `[0,5,5]` bestanden (jetzt voller Kurvenvergleich); der Padding-Test erzeugte seine Erwartung selbst und konnte **strukturell nicht rot werden** (raus); `analyzeMoonEffect` war ungetestet trotz Off-by-one-Fix in #271. **Neu `scripts/verify_twins_mutation.py`** — prüft den WÄCHTER, nicht den Code: baut 12 bekannte Fehler absichtlich wieder ein und verlangt Rot. Beim ersten Lauf blieb **einer unbemerkt**, fehlender Testfall ergänzt, jetzt 12/12. **Lessons:** eine selbstkalibrierende Referenz braucht eine **Sperre gegen Zirkelschluss** (unabhängige Quelle oder gemessene Plausibilitätsgrenze); ein Test auf **ein** Element lässt die halbe Fehlerklasse durch; **den Wächter mit Mutationen prüfen** — sonst ist „der Test besteht" eine Aussage über den Test, nicht über den Code.
> Version 58.0 | 2026-09-11 | **Welle-1-Abnahme durch externen Reviewer: 7 weitere Befunde, davon 2 eigene Überkorrekturen (kanonisch: [docs/KERNMETHODIK.md](docs/KERNMETHODIK.md), PR #272)** — Die 12 Korrekturen aus Welle 1 gingen zur Abnahme: 7 bestätigt, 5 unvollständig. **Schwerster Fund, aus Welle 1 komplett übersehen: `analyzeTurnOfMonth` gruppiert nach `y + '-' + m` OHNE Nullpadding** und nimmt den nächsten *sortierten* Schlüssel als Folgemonat — `"2024-1" < "2024-10" < "2024-2"`. Der **Januar zog Oktober-Daten, der Dezember Februar, der September den Januar des Folgejahres**: 3 von 12 Monaten falsch, live auf `/monatswechsel`. Python rechnet `next_month = month + 1` explizit und war korrekt — Welle 1 schaute auf die *Kumulation*, nicht auf die *Fensterbildung*. **Zwei Überkorrekturen der eigenen Fixes:** `calculate_period_stats` verlangte Beobachtungen bis Kalendertag 365, aber ein abgeschlossenes Jahr endet fast nie am 31.12. (XETRA schliesst am 30.12., NYSE 2006/2017/2023 am 29.12.) → die Regel warf **7 von 26 NYSE- und 15 von 26 XETRA-Jahren** weg; neu `year_end_reference()`/`year_covers()`, Referenz ist **wie weit ein vollständiges Jahr DIESES Tickers reicht** (selbstkalibrierend, ohne Börsenkalender). Und: JS setzte bei kaputtem `log_return` still `lr = 0`, Python verwarf das Jahr → jetzt verwerfen beide. **Drei weiter ungefilterte Stellen:** `jahreszyklus.html::periodStats` filterte gar nicht (Seite zeigte andere Trefferquoten als das Backend), der Zyklus-Zweig trug die **YTD**-Rendite des laufenden Jahres als volle Jahresrendite ein, und der Blog-Monatszyklus zählte im September Okt–Dez als exakt 0 %. **`calc_tdom_range_return`** gruppierte weiter ohne Ticker (dritte Fundstelle derselben Klasse). **Wächter grundlegend umgebaut:** er verglich Python gegen einen *handgeschriebenen Nachbau* der JS-Logik — **Block 4 führt jetzt die echte `seasonal-compute.js` in node aus** (`scripts/js/twin_probe.js`, `window`-Stub genügt); neu abgedeckt sind `interpolate_to_365` und `analyze_turn_of_month`, beide vorher ungeprüft. **Lessons:** **ein Fix kann schlimmer sein als der Fehler** — wer eine Bedingung verschärft, muss messen, wie viel sie wegwirft; **ein Wächter, der einen Nachbau prüft, prüft eine Fiktion** — und man muss ihn absichtlich kaputt machen, um zu sehen, ob er rot wird (ein zwischenzeitlicher textueller Quellcode-Check bestand, während der Fehler drin war: er traf einen unbeteiligten Datums-Helfer).
> Version 57.0 | 2026-09-10 | **Review-Welle 1 abgeschlossen: Kern-Methodik (12 Befunde, alle behoben — kanonisch: [docs/KERNMETHODIK.md](docs/KERNMETHODIK.md), PRs #265-270)** — Der schwerste Befund saß in `normalize_year`: kumuliert wurde `Σ r_0..r_{j-1}` statt `Σ r_1..r_j`, also landete der **Jahreswechsel-Return IM Jahr** (+10 % Auftakt → Tag 2 stand auf 110 statt 101) und der letzte Handelstag fiel weg. `log_return` ist rückwärts definiert (`LN(close/prev_close)`), gehört also zu SEINER Zeile — das Frontend rechnete längst richtig, die **Zwillinge lieferten systematisch verschiedene Kurven**. Weiter: **Schaltjahre verloren den 31.12.** (`range(1,366)` gab Tag 366 nie aus — still, nur alle 4 Jahre); **TDOY/TDOM zählte gespeicherte Zeilen statt Kalendertage** (Historie ab 1. Juli → `tdoy=1` statt 124; Lücke schob alle Folge-Indizes); **`tdoy_calc` im Frontend zählte auf GEFILTERTEN Zeilen durch** → Top-25-Tabelle gruppierte unter falschem Handelstag; **`calculate_period_stats` zählte das laufende Jahr als abgeschlossen** (Periodenende = fortgeschriebener Kurs) während das Frontend es ausschließt; **ein einzelnes NaN-`log_return` machte über `exp(cumsum)` den Saison-Durchschnitt ALLER Jahre zu NaN**; **`last_actual_day` wurde vom gemeinsamen Builder nie gesetzt** → die vier `|| 365`-Schutzabfragen im Dashboard waren toter Code; **Constant-Fill floss in Anomalie-Konfidenz, veröffentlichte Blog-Heatmap (0,00 % statt Lücke) und Signifikanztest** → neuer zentraler Helfer `shared/calculations.last_actual_day(yd)`, bewusst filtert der **Konsument**, nicht der Erzeuger; **TDOM-Zähler und `shift(-1)` liefen über Ticker-Grenzen hinweg** (Overnight-Rendite AAPL-Open gegen SAP-Open); **`backfill_tdoy` verschluckte Upsert-Fehler** (Exit 0, Monitoring grün, Zeilen veraltet). **Selbst gefunden, nicht im Reviewer-Bericht: Turn-of-Month zeigte jede Bewegung einen Handelstag zu spät** — und zwar in **beiden** Zwillingen identisch, der JS-Kommentar sagte „1:1 wie Python". ⚠️ **FOLGE: die veröffentlichten ToM-Zahlen inkl. Backtest „SPY Down-Month ToM Reversal" (v47) müssen neu gerechnet werden.** Neuer Wächter **`scripts/verify_seasonal_twins.py`** (6 Fälle, vergleicht Backend + Frontend-Logik + analytische Sollkurve, Exit 1 bei Drift). Abgeleitete Caches nach dem Normalisierungs-Fix neu gerechnet (366 Ticker, 364 OK / 0 Fehler). **Lessons:** **Übereinstimmung zweier Zwillinge ist kein Beleg für Richtigkeit** — ein Vergleich braucht eine dritte, unabhängig hergeleitete Referenz; Off-by-one in kumulierenden Reihen zeichnet **plausible** Kurven → Testfall bauen, in dem sich genau EIN Tag bewegt; eine Schutzabfrage gegen ein nie gesetztes Feld ist schlimmer als keine; externe Befunde **gegenprüfen** (der Reviewer nannte Befund 1 neutral „einigt euch auf eine Konvention" — ungeprüft umgesetzt hätte es das korrekte Frontend zerstört).
...
### 🔴 SOFORT — Funktional (aus Review-Welle 1)
- [ ] **`SPY Down-Month ToM Reversal` nachoptimieren (Einstiegstag), NICHT neu rechnen.** Präzisierung 2026-09-10 nach Gegenprüfung der eigenen Folgenabschätzung aus PR #270: `calc_downmonth_tom` nutzt `_nthTradingDay` + echte Closes, **nicht** die kumulierte Reihe → die Kennzahlen (Sharpe 0,34 · WR 72 % · PF 2,39) sind **korrekt gerechnet**. Offen ist etwas anderes: die **Parameter** (Einstieg TDOM 14, Haltedauer 13) wurden aus der um einen Handelstag verschobenen ToM-Kurve abgelesen und können danebenliegen. → Grid über TDOM 12-16 × Haltedauer 11-15 auf der korrigierten Kurve, dann entscheiden ob 14/13 bleibt. Zahlen bleiben bis dahin zitierfähig, wenn der offene Einstiegstag dabeisteht. Hintergrund: [docs/KERNMETHODIK.md](docs/KERNMETHODIK.md)
- [x] **Endabnahme Flows/Optionen** (PR #275, 2026-09-11) — 2 lückenhafte Fixes nachgezogen (`--vol-pctl`-Funktionsdefaults, cm-Zeilen ohne `iv_atm` im Backfill), eine Reviewer-Einstufung mit Messung korrigiert. Detail: [docs/OPTIONS.md](docs/OPTIONS.md#endabnahme-2026-09-11-pr-275).
- [x] **Endabnahme Kern-Methodik: FREIGEGEBEN** (PR #274, 2026-09-11) — beide Auflagen erfüllt, XETRA-2023 aus Rohkursen gegengerechnet (0,0000000000 pp), Mutationstest 15/15. Betrieb der Review-Schleife: [docs/REVIEW_MIT_CODEX.md](docs/REVIEW_MIT_CODEX.md).
- [x] **Dritte Prüfrunde (PR #273)**: Zirkelschluss in `year_end_reference` behoben, Wächter per Mutationstest belegt (`scripts/verify_twins_mutation.py`, 12/12). Details im Versionskopf v59.0 + [docs/KERNMETHODIK.md](docs/KERNMETHODIK.md) 4c.
- [x] **Welle-1-Abnahme (PR #272)**: 7 weitere Befunde behoben, davon 2 eigene Überkorrekturen. Details im Versionskopf v58.0 + [docs/KERNMETHODIK.md](docs/KERNMETHODIK.md) Abschnitt 4b.
- [x] **Off-by-one-Fehlerfamilie geschlossen** (PR #271): das Muster saß an **fünf** Stellen, PR #270 fixte zwei. Nachgezogen: `monatswechsel.html::buildCurrentYearTOMCurves` (dritte ToM-Kopie, laufendes Jahr — lag im selben Chart neben der historischen Kurve), `seasonal-compute.js::analyzeMoonEffect` (live `/mondphasen`), `scripts/video/render_lunar_charts.py`, `shared/holidays.py`, `pages/06_Mondphasen.py` + `pages/03_Monatszyklus.py`. **Lesson:** nach einem Fix an geteilter Mathematik per `grep` auf die **Code-Form** suchen, nicht auf den Funktionsnamen.
...
- [x] **Welle 1 — Kern-Methodik: normalisierte Renditen + Zeitindizes** (2026-09-10,
  12 Befunde, alle behoben — **kanonisch: [docs/KERNMETHODIK.md](docs/KERNMETHODIK.md)**,
  PRs #265-270). Die Leitfrage („rechnen Backend und Frontend nachweislich dasselbe?")
  traf ins Schwarze: die Normalisierung war im Backend um eine Zeile verschoben.
  Neuer Wächter `scripts/verify_seasonal_twins.py` hält die Zwillinge deckungsgleich —
  und prüft **beide** gegen eine dritte, analytisch hergeleitete Referenz, weil der
  Turn-of-Month-Fehler in beiden Implementierungen identisch steckte.
  ⚠️ Offene Folgearbeit siehe unten (ToM-Zahlen neu rechnen).

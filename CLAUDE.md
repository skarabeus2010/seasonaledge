...
| Dashboard | `/dashboard` | app, charts, holidays, seasonal-compute, decade-compute, strategy-compute, streak, fomc-dates | **Ticker-Uebersicht** in Bento-Grid (11 Cards). Hero: Trading Day Header + Titel. Cards: (1) **KI-Score** Composite 0-10 mit 4 Sub-Scores (Musterpfad, Trend, Win-Rate, Tracking) + Signal-Badge Bullish/Neutral/Bearish, (2) **Crash-Ampel** ticker-individuell via `computeRegime(rows)` — Vol5/Vol20/DD/Return-Features vs 252d-Percentile des eigenen Tickers → Traffic-Light + Risk-Score 0-100, (3) **Anomalie-Radar** via `SA.decadeCompute.fromPrices`, (4) **Year-Chart** Ø + 25./75. Bands + aktuelles Jahr + Heute-Marker, (5) **Month-Chart** Intra-Monat TDOM Ø + aktueller Monat + we-are-here, (6) **TruePath** Top-N Match-Jahre + Forward-Projektion, (7) **Wochentag-Check** Win-Rate + Ø Rendite + TDOM-Position, (8) **Risiko-KPIs** 4 Cards (aktueller DD, Ø max DD, aktuelle Vola, Vola-Percentile), (9) **Trifecta** SCR+FFD+JanB auf Ticker angewendet mit Disclaimer, (10) **Top 5 Plain Vanilla Strategien** — nur mit Signal in naechsten 30 Tagen, sortiert nach Datum, aktuelle Streak als Badge (🟢 3 W / 🔴 5 L), (11) **Naechste Events** (FOMC/OPEX/Vollmond/Holiday) mit historischer Streak + Win-Rate + Ø Return pro Ticker via `analyzeEventWindow` (t-3 bis t+3). Sidebar minimal: Ticker + Years 5-30 (default 15). URL-Param `?t=TSLA`. **Lazy-Render** fuer Strategien (50ms) + Events (100ms) — Initial ~1-2s. Alle Math-Helper inline im `_SA_DASH` Modul (~900 Zeilen). Crypto: FOMC+Moon funktionieren, OPEX/Holiday werden uebersprungen. |
...
- [x] **2026-04-10** **Mobile-Responsiveness komplett** — 6 Commits vom Büro aus:
  - `046f567` **KPI/Tabellen/Anomalie-Radar Mobile-Rules**: `.kpi` 2-spaltig ≤640px / 1-spaltig ≤380px, Tabellen horizontal scrollbar ≤768px, Sidebar-Inputs 16px (iOS Auto-Zoom Fix), Anomalie-Radar Grid 2×3 ≤640px mit Tooltip-Repositionierung + Touch-Focus-Fallback, Dashboard-Bento Zwischen-Breakpoint 900px, Backtest Tab-Nav `flex-wrap`
  - `a77ed10` **Root-Cause Sidebar-Collapse**: `body.sa-sidebar-collapsed` Regeln hatten höhere Spezifität (0,2,1) als die Narrow-Mode-Regeln (0,1,0) und haben sie auf Mobile überschrieben → Grid blieb `0 1fr`, main landete in 0px-Spalte → Trading-Day-Header schrumpfte auf 34px. Fix: alle Collapsed-Regeln in `@media (min-width: 1280px)` gekapselt + Narrow-Mode Grid + Toggle mit `!important` als Specificity-Versicherung
  - `e4a904f` **Page-Title Selektor-Fix**: `:first-child` matcht nicht wenn main mit loading-overlay/error-banner beginnt → Selektor vereinfacht auf `.app main .page-title`
  - `b75d03f` **Nginx Cache-Strategy**: CSS/JS bekommen `max-age=0, must-revalidate` + ETag via Regex-Location statt `max-age=86400`. Mobile Safari cached CSS nicht mehr 24h, Deploys wirken sofort
  - `8338207` **Cache-Busting per Git-SHA**: `inject_credentials.sh` hängt bei jedem Deploy `?v=<git-short-sha>` an alle `/landing/css/*.css` und `/landing/js/*.js` Refs in HTML. Browser sieht neue URL → kann nicht aus Cache antworten. Existierende `?v=...` Versionen werden überschrieben
  - `c9d796a` **Burger-Menu Fix**: `dashboard.html`, `monatszyklus.html`, `backtest-engine.html`, `wochentage.html` haben nav.html per manuellem `fetch()` geladen statt `loadComponent()` → `initNav()` nie aufgerufen → Burger ohne Click-Handler. Zusätzlich: `.nav__links` Container kollabierte auf 64px (display:flex+position:fixed Browser-Quirk) → expliziter `height: calc(100dvh - var(--nav-h))` + `overflow-y:auto` + Touch-Sizing für alle Items (16px Font, .85rem Padding, full-width), sub-Items mit `border-left` Akzent für visuelle Hierarchie
- [x] **2026-04-10 Nachmittag** **Blog-Suche** (Commit `6a4f7a5`) — Client-side Live-Search im `blog/templates/blog_index.html` Template. Search-Input mit Lupe-Icon, AND-Logik bei Whitespace-Terms, Counter „X von Y Artikeln", Empty-State, Keyboard-Shortcuts (`/` fokussiert, `Esc` leert), URL-Persistenz via `?q=...`. Filtert per `data-search` Attribut das title+description+category+tags+ticker lowercased kombiniert. Wirkt automatisch auf alle 4 Index-Pages (Haupt + Education + Marktausblick + Tutorials). ~90 Zeilen Vanilla-JS, keine Dependencies, skaliert bis ~1000 Posts.
- [x] **2026-04-10 Nachmittag** **Footer-Redesign komplett** (4 Commits: `ba074af` Landing + `18e18ef` Globale app.css/footer.html + `69fa3e4` 5-col Grid Medium-Range + siehe V4):
  - Landing `index.html`: 6-col Grid (`1.6fr repeat(5,1fr)`), Brand + 5 Feature-Cols in einer Reihe. Risikohinweis aus `.wrap` raus in neues `.foot__disclaimer` div → spannt komplette Viewport-Breite. Twitter-Badge `X @SeasonAlph4882` unter dem Brand-Block mit X-Icon + Hover-Gold-Border.
  - `app.css` + `components/footer.html`: Gleiches Design auf alle 21 Feature-Pages portiert. Neue Breakpoint-Kaskade (≥1400px 6-col, 900-1399px Brand-fullwidth + 5 Cols, 600-899px 3-col, <600px 2-col, <380px 1-col). Gaps von 2rem auf 1-1.25rem reduziert.
  - Footer-Grid-Layout-Fix v2: Medium-Range 900-1399px hatte im ersten Fix `1.4fr + repeat(3,1fr)` → Mehr wrappte auf Row 2, Rechtliches auf Row 3 alleine. Behoben mit `repeat(5,1fr)` → alle 5 Feature-Cols immer in einer Reihe, Rechtliches ganz rechts.
- [x] **2026-04-10 Abend** **SEO-Foundation-Layer komplett** — 9 Commits, alle Quick-Wins abgefeuert:
  - `669e13a` **OG-Image + Meta-Tags + Sitemap v1**: `scripts/generate_og_images.py` (Pillow-basiert) generiert og-image.png 1200×630 (V3 Ultra Dark + Gold-Akzent-Bar, „SeasonAlpha"-Branding, Tagline, 3 KPI-Cards 131J/15M/500+), apple-touch-icon.png 180×180, favicon-16x16/32x32. Fixt: vorher waren og-image.png und apple-touch-icon.png als 404er referenziert → keine Social-Share-Thumbnails. Sitemap von 276 auf 319 URLs erweitert (21 Feature-Pages mit Prioritäten 0.7-1.0, 4 Blog-Indizes, 18 Blog-Posts dynamisch aus `blog/posts/*.md` Frontmatter). `scripts/upgrade_page_meta.py` (idempotent, Marker V2) stockt Meta-Tags auf allen 21 Pages auf: author/robots/theme-color, kompletter Open-Graph + Twitter-Card Block, 4 Icon-Links, JSON-LD WebPage-Schema mit publisher + logo. Blog-Post Article-Schema aufgebohrt (Google-News-eligible): datePublished/Modified mit ISO-Timezone, publisher.logo als ImageObject, mainEntityOfPage als Object, articleSection, inLanguage, article:* Meta-Tags, twitter:site=@SeasonAlph4882.
  - `bbe79c9` **SEO_MARKETING.md Living Doc** — ergänzt SEO_ENGINE.md um Marketing-Status, Priority-Buckets (SOFORT/DIESE-WOCHE/MITTELFRISTIG/LATER), GSC-Workflow, Bing-Setup, Monitoring-Cadence, Validator-Tool-Links.
  - `cada00f` **robots.txt AI-Policy + Breadcrumbs v1 + Sitemap-Auto-Discovery** — Entscheidung 2026-04-10: AI-Crawler DÜRFEN indexieren (Brand-Awareness via ChatGPT/Claude/Perplexity). Explizite Allow-Regeln für 15 AI-Crawler (GPTBot, ClaudeBot, PerplexityBot, Google-Extended, CCBot, Cohere, Meta, Apple, ByteDance, Mistral) + Crawl-delay 10s für Semrush/Ahrefs/MJ12. Breadcrumb-JSON-LD auf allen 21 Pages (V3-Marker). Sitemap `build_sitemap()` mit `glob('landing/pages/*.html')` Auto-Discovery + PRIORITY_OVERRIDES Dict + PAGE_EXCLUDES (_disabled, 404, index, apex-demo, unsubscribe) — neue Pages fliessen automatisch in die Sitemap, keine manuelle Liste mehr pflegen. Schliesst die TODO-Lücke aus SEO_MARKETING.md.
  - `1d8d958` **Breadcrumb Validator-Fix v2** — Google Rich Results Validator meldete „Feld 'item' fehlt" bei 3-Level Breadcrumbs (Home → Zyklen → Page) weil Kategorie-Zwischenschritt keine eigene URL hatte. Fix: 2-Level Breadcrumbs (Home → Page), beide mit `item`-URL. MARKER V3 → V4, alle 21 Pages re-upgraded.
  - `85f23b1` **IndexNow + SearchAction Schema + optimierte Titles** — `scripts/optimize_page_titles.py` (neu) ersetzt die 21 generischen Titles ("Jahreszyklus — SeasonAlpha") durch SEO-optimierte Varianten ("Jahreszyklus — Saisonale Jahresmuster am Markt | SeasonAlpha"), alle <62 Chars und <170 Chars Description mit Ziel-Keywords. MARKER V4 → V5, upgrade_page_meta.py re-run. Landing `index.html` bekommt WebSite + SearchAction JSON-LD (`urlTemplate: /dashboard?t={search_term_string}`, für Google Sitelinks-Searchbox + Bing/LLM-Kontext). Organization bekommt `sameAs: [twitter]`. Umlaute im bestehenden Landing-JSON-LD von `oe/ue/ss` auf Unicode-Escapes. IndexNow Script (`scripts/submit_indexnow.py`, pure stdlib) mit 26 Core-URLs-Default + --all + --urls + --dry-run, ins `deploy.yml` integriert als non-blocking Step. Key-File `landing/assets/indexnow-key.txt` (32-hex).
  - `8526ece` + `b628b55` **IndexNow Path-Fix** — Bing verlangte Key am Host-Root `/{key}.txt`. nginx `location = /c0d4540dc5a10d464d473960f4d20be3.txt` mit alias auf landing/assets/. Erste IndexNow-Submission HTTP 200 erfolgreich.
  - `99d145a` **www → non-www 301 Canonicalization** — beide `www.` und `non-www` Versionen lieferten vorher HTTP 200 mit identischem Content (Duplicate-Content-Problem). Neue nginx server-Blöcke für Port 80 + 443 mit `server_name www.seasonalpha.ai` die via `return 301 https://seasonalpha.ai$request_uri` canonisieren. SSL-Zertifikat deckt beide Domains (Subject Alternative Names verifiziert).
  - `088001e` **Programmatic /analyse/ Pages auf noindex + aus Sitemap raus** — GSC hat 72/270 der programmatischen Landing-Pages als „Gefunden – zurzeit nicht indexiert" markiert (thin content ~300 Wörter, fake KI-Prognose mit Dummy-Daten die auf allen 270 Pages identisch waren, Chart-Placeholder mit Emoji). Das sind klassische Scaled-Content-Abuse-Signale aus Googles März-2024 Policy. Fix (Weg A, reversibel): `seo_template.html` bekommt `<meta name="robots" content="noindex, follow">`. `seo/programmatic_seo_builder.py::build_sitemap()` mit `ENABLE_PROGRAMMATIC_IN_SITEMAP = False` → Sitemap schrumpft von 319 auf **49 URLs**. Die 270 HTML-Files existieren weiterhin im `seo/output/` und sind direkt aufrufbar (für interne Links, Altlasten-Support), aber Google indexiert sie nicht mehr und die Sitemap bewirbt sie nicht mehr. Ziel: Domain-Quality-Score-Drag durch Thin-Content-Pollution weg, Rest der Site (21 Feature-Pages + Blog) kriegt in 2-4 Wochen bessere Rankings. Re-Aktivierungs-Pfad dokumentiert im Commit-Body für Weg B (echter Content: Monats-Perf-Tabelle aus Supabase, SVG-Chart, Dekaden-Split, Asset-Klassen-Intros, App-CTAs).
- [x] **2026-04-10 Spät-Abend** **Feature #3 Saisonal-Scanner LIVE** — 4 Commits, MVP Frontend + Full-Backend + Weekly Cron:
  - `632e5d6` **Scanner MVP Frontend** `landing/pages/scanner.html` (~450 Zeilen): Standard SA-Layout (Sidebar + Main), 4 Summary-Cards (Total/Bullish/Neutral/Bearish mit %), sortierbare Ergebnis-Tabelle (Rang/Ticker/Name/Kategorie/Signal/Score/Win-Rate/Ø Return/Dashboard-Link), Click auf Ticker → `/dashboard?t={ticker}`, Header-Click toggelt Sort asc/desc, Empty-State wenn keine Ticker passen, Methodik-Expander. **Sidebar-Filter** (alle client-side, instant): Such-Input (Ticker+Name), Signal-Checkboxen mit Live-Count, Score-Range Min-Slider 0-10, Win-Rate Min-Slider 0-100%, Kategorie-Multi-Select mit Master-Toggle + Counts, Reset-Button. Daten-Pipeline: fetch `tickers.json` (Metadata) + fetch Supabase `scanner_results` (neuestes scan_date, order by score desc), client-side Join + Filter + Sort. Plus: `scripts/generate_tickers_json.py` (regeneriert `landing/data/tickers.json` aus `shared/symbols.py` mit UTF-8 + kategorie-Feld, vorher war latin-1 ohne Kategorie). Plus: Integration in `nav.html`, `index.html` (Top-Level-Link + Strategien-Dropdown + Footer-Spalte), `deploy/nginx.conf` (`/scanner` Route), `scripts/upgrade_page_meta.py` PAGE_META, `scripts/optimize_page_titles.py` PAGE_SEO, `seo/programmatic_seo_builder.py` PRIORITY_OVERRIDES (0.95 daily). Sitemap 49 → 50 URLs.
  - `4da4e3a` **Supabase-Credential Injection-Line nachgezogen** — scanner.html hatte `%%SUPABASE_URL%% / %%SUPABASE_ANON_KEY%%` Platzhalter-Zeile vergessen → „Supabase nicht konfiguriert" Error. Fix: 1 Inline-Script vor `app.js`-Tag.
  - `606fd40` **Full Scanner Weekly Cron + Pro-Ticker-Upsert**. Root-Cause-Analyse: `scanner_results` hatte nur 94/270 Ticker vom 2026-03-31 weil (a) `nightly_refresh.yml` hatte `command_timeout: 10m` + `timeout-minutes: 15` und der Loop braucht für 270 Ticker ~20-40 Min → SSH wurde gekappt → Python-Prozess abgebrochen, und (b) `refresh_ticker_data()` sammelte alle Scanner-Results im Memory-Array und machte `store_scanner_results()` erst am Ende → bei Timeout alle Results verloren. **Fix 1**: `scripts/full_scanner_run.py` (neu, ~280 Zeilen) — dedicated Scanner-Script mit Pro-Ticker-Upsert (resume-safe), `--resume` (skip bereits heute gescannte), `--limit N`, `--only T1,T2`, `--quick/--full`, `--progress-every N` mit ETA, strukturiertes Per-Ticker-Log, Exit-Code mit Success-Rate. **Fix 2**: `.github/workflows/full_scanner.yml` (neu) — Weekly Cron Sonntag 03:00 UTC + `workflow_dispatch` mit quick/limit/resume Inputs, `timeout-minutes: 90`, `command_timeout: 85m`. **Fix 3**: `nightly_refresh.py` bekommt auch den Pro-Ticker-Upsert nachgerüstet (5 Zeilen Diff innerhalb des KI-Score Compute-Blocks). **Fix 4**: `nightly_refresh.yml` Timeout 15/10m → 60/55m. **Live-Verifikation**: Der erste Full-Scanner-Run hat **269 von 270 Tickern** erfolgreich verarbeitet (nur 1 ohne ausreichende Historie). Neue Top-5 Bullish: NESN.SW (Nestlé, Score 7.5, WR 95.2%), INVE-B.ST, EQIX, SU.PA, XLE. Das ist ein kompletter Ersatz der 94 veralteten Ticker vom 2026-03-31. `/scanner` zeigt jetzt automatisch die vollen Daten.
- [x] **2026-04-10 Abend 2** **Feature #4 Watchlist Phase 1 (Local-First MVP) LIVE** — neue Module + Page + Integrationen:
  - `landing/js/dash-compute.js` (neu, ~260 Zeilen) — Dashboard Compute-Kern extrahiert: `findMatchingYears`, `computeTruePath`, `computeKiScore`, `computeRegime` + math/time helpers (mean/stdev/corrcoef/todayDoy/etc). Wiederverwendbar von `/watchlist` ohne dashboard.html laden zu müssen. Math/Rendering strikt getrennt.
  - `landing/js/watchlist.js` (neu, ~290 Zeilen) — `SA.watchlist` API: localStorage-backed (`sa-watchlist-v1`), Schema v1 mit version/created_at/updated_at/items/sort, max 50 Ticker, Cross-Tab-Sync via storage-Event, Internal Event-Listeners (changed/added/removed) + window.dispatchEvent. Migration-ready (`_export`, `_import`, `_backend`, `_schemaVersion`) für späteren Supabase-Upgrade in Feature #4 Phase 2. API: get/has/add/remove/toggle/clear/count/on/off/setSort/getSort. Case-insensitive Ticker-Normalisierung. 16 API-Tests in isolation Node-Test grün.
  - `landing/pages/watchlist.html` (neu, ~420 Zeilen) — Komplette Page mit Sidebar (Add-Input + Sort + Signal-Filter + Clear-Button) + Main (Compact-Card-Grid + Empty-State + Summary-Stats). Parallel-Load aller Watchlist-Ticker via `Promise.all(items.map(computeForTicker))`. Compact-Card zeigt Ticker+Name, KI-Score + Signal-Badge, Crash-Ampel-Dot + Label + Risk-Score, Win-Rate + Return 5d KPIs, Dashboard-CTA. Toast-System für User-Feedback. Cross-Tab-Sync Listener. `robots: noindex, nofollow` weil personalisiert.
  - `landing/pages/dashboard.html` erweitert: `.wl-toolbar` in `.dash-head` mit Toggle-Button (Zur Watchlist / In Watchlist) + Count-Badge-Link zur `/watchlist` Page. CSS für `.btn-wl-toggle` + `#wl-badge`. JS: `updateWatchlistButton()` bei Ticker-Wechsel + Event-Listener auf `SA.watchlist.on('changed')`. Script-Tag für `watchlist.js`.
  - `landing/pages/scanner.html` erweitert: Per-Row ⭐-Button in der Actions-Spalte (SVG-Star, `.btn-wl-row`, `.active` Style bei In-Watchlist). Event-Delegation via `initWatchlistDelegation()` — klick toggelt, sync mit `SA.watchlist.on('changed')`. CSS: `.scanner-table td.actions` als flex-box, `.btn-wl-row` 28×28 Icon-Button mit gold-Border bei active.
  - `landing/components/nav.html` + `landing/index.html` (inline Nav): Neuer `/watchlist` Link nach `/scanner`.
  - `deploy/nginx.conf`: `location = /watchlist` rewrite auf `landing/pages/watchlist.html`, Cache `max-age=3600`.
...
### ✅ ERLEDIGT — Mobile Responsiveness (06 Commits am 2026-04-10 Vormittag)
Siehe „Erledigt diese Woche" oben. Alle 10 ursprünglich identifizierten Bugs + 3 zusätzlich im Live-Test gefundene Root-Cause-Bugs sind behoben. Mobile-Fixes greifen automatisch auf alle 21 Pages dank globaler `app.css` + Shared-Renderer-Pattern.

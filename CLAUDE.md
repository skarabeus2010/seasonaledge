...
- **SSH aus der Claude-Umgebung GEHT** — der Key heißt aktuell **`~/.ssh/flatradar`** (verifiziert 2026-09-08; ein `~/.ssh/mietwatch` existiert dort **nicht** mehr, der alte Eintrag hier führte in die Irre und kostete eine halbe Session manueller PowerShell-Eingaben): `ssh -i ~/.ssh/flatradar -o BatchMode=yes root@178.104.75.46 "…"` (Default-`ssh` bietet den Key nicht an → `-i` explizit). **Vor dem Aufgeben immer `ls ~/.ssh/` prüfen**, statt dem hier notierten Namen zu vertrauen. Der Auto-Mode-Klassifikator kann schreibendes SSH blocken → dann `Bash(ssh -i ~/.ssh/mietwatch:*)` in `.claude/settings.local.json` allow. **NUR `/opt/seasonaledge` anfassen, nie `/opt/mietwatch`, `/opt/flatradar` (mietwatch/Wohnungsbot) — Dateien + DB.** Damit sind Server-Fixes (Git/nginx/docker) selbst machbar, nicht nur „macht der User".

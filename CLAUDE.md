...
- [ ] **Turn-of-Month-Zahlen neu rechnen — die veröffentlichten Werte sind um einen Handelstag verschoben.** Der Fix (PR #270) korrigiert die Kumulation in `analyze_turn_of_month` (Python) und `analyzeTurnOfMonth` (JS); jede vorher publizierte ToM-Aussage entstand aber noch mit dem Versatz. **Betroffen:** der Backtest **„SPY Down-Month ToM Reversal"** (CLAUDE.md v47: Sharpe 0,21→0,34 · WR 68→72 % · PF 1,80→2,39), `landing/js/strategy-compute.js::calc_downmonth_tom`, die `SA.STRATEGIES`-Kategorie `monat`, das Dashboard-Signal und die zugehörigen Blog-Posts (DE+EN). **Bis zur Neuberechnung diese Zahlen nicht weiter zitieren.** Hintergrund: [docs/KERNMETHODIK.md](docs/KERNMETHODIK.md#411-turn-of-month-war-einen-handelstag-zu-spät--pr-270)

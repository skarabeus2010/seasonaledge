...
MUTATIONEN = [
    ...
    ("Laufzeit", "Kennzahl wieder halbe Intervallbreite statt groesster Abstand (Codex R3)", SK,
     "max(sk_pub - lo_r, hi_r - sk_pub)", "(hi_r - lo_r) / 2", "laufzeiten"),
    ("Laufzeit", "Kennzahl wieder um den ungerundeten statt den ausgegebenen Skew (Codex R4)", SK,
     "                sk_pub = r[\"skew_ne_pts\"]",
     "                sk_pub = (leg_ne[\"put_iv\"] - leg_ne[\"call_iv\"]) * 100.0", "laufzeiten"),

...
def holiday_path(data, name, hol_name, out, lang="de"):
    en = lang == "en"
    path = np.asarray(data["path"], float)
    labels = ["t−3", "t−2", "t−1", "t0", "t+1", "t+2", "t+3"]
    fig = _fig()
    title = (f"{name} — {hol_name}: path around the holiday" if en
             else f"{name} — {hol_name}: Verlauf um den Feiertag")
    sub = (f"cumulative return, normalized (t−3 = 0) · {data['n']} years" if en
           else f"kumulierte Rendite, normiert (t−3 = 0) · {data['n']} Jahre")
    fig.text(0.5, 0.94, title, color=WHITE, fontsize=20, fontweight="bold", ha="center")
    fig.text(0.5, 0.885, sub, color=MUTED, fontsize=13, ha="center")
    ...
    ax.text(3, path.max() * 1.02 + 0.05, "Holiday" if en else "Feiertag", color=WARM, fontsize=12, ha="center")
    ax.set_xticks(x); ax.set_xticklabels(labels)
    ax.yaxis.set_major_formatter(FuncFormatter(lambda v, _: f"{v:.1f}%"))
    ax.set_xlim(-0.2, 6.2)
    fig.text(0.99, 0.02, f"seasonalpha.ai · {'Data' if en else 'Daten'}: {data['data_date']}",
             color=MUTED, fontsize=9, ha="right")
    fig.savefig(out, facecolor=BG); plt.close(fig)


def significance_gauge(data, name, hol_name, out, lang="de"):
    en = lang == "en"
    sig = data["significant"]
    col = GREEN if sig else RED
    rel = data["relevance"]
    fig = _fig()
    title = (f"Significance Test — {name} · {hol_name}" if en
             else f"Signifikanz-Test — {name} · {hol_name}")
    sub = ("Is the window return statistically different from zero?" if en
           else "Ist die Fenster-Rendite statistisch von Null verschieden?")
    fig.text(0.5, 0.93, title, color=WHITE, fontsize=20, fontweight="bold", ha="center")
    fig.text(0.5, 0.875, sub, color=MUTED, fontsize=13, ha="center")
    ...
    label = (("Significant" if sig else "Likely random") if en
             else ("Signifikant" if sig else "Zufall wahrscheinlich"))
    ax.text(0, -0.08, label, color=col, fontsize=17, fontweight="bold", ha="center")
    if en:
        stats = (f"t = {data['t']:.2f}      p = {data['p']:.4f}      Ø +{data['avg']:.2f} %"
                 f"      Win {data['win']:.0f} %      n = {data['n']}")
        foot = "Relevance = 50%·(1−p) + 30%·win rate + 20%·effect size (Cohen’s d)"
    else:
        def _c(x, dec=2):
            return f"{x:.{dec}f}".replace(".", ",")
        stats = (f"t = {_c(data['t'])}      p = {_pstr(data['p'])}      Ø +{_c(data['avg'])} %"
                 f"      Win {data['win']:.0f} %      n = {data['n']}")
        foot = "Relevance = 50%·(1−p) + 30%·Win-Rate + 20%·Effektstärke (Cohen’s d)"
    fig.text(0.5, 0.12, stats, color=WHITE, fontsize=15, ha="center")
    fig.text(0.5, 0.05, foot, color=MUTED, fontsize=10, ha="center")
    fig.savefig(out, facecolor=BG); plt.close(fig)
...
def main():
    ...
    sfx = "-en" if a.lang == "en" else ""
    holiday_path(data, _disp(a.ticker), h[2], out_dir / f"holiday-verlauf{sfx}.png", a.lang)
    significance_gauge(data, _disp(a.ticker), h[2], out_dir / f"signifikanz{sfx}.png", a.lang)
    print(f"[blog-charts] OK ({a.lang}) -> {out_dir}")
